...
DEFAULT_SOURCES: List[Source] = [
    Source("Reuters Business", "https://feeds.reuters.com/reuters/businessNews", "news", 0.95),
    Source("Reuters World", "https://feeds.reuters.com/Reuters/worldNews", "news", 0.95),
    Source("CoinDesk", "https://www.coindesk.com/arc/outboundfeeds/rss/", "news", 0.85),
    Source("The Block", "https://www.theblock.co/rss.xml", "news", 0.85),
    Source("Federal Reserve Press", "https://www.federalreserve.gov/feeds/press_all.xml", "official", 1.0),
    Source("SEC Press", "https://www.sec.gov/news/pressreleases.rss", "official", 0.98),
    Source("U.S. Treasury", "https://home.treasury.gov/news/press-releases/rss", "official", 0.97),
]


BULLISH = [
    "etf inflow", "approval", "rate cut", "easing", "institutional buy", "adoption", "record high",
    "short squeeze", "accumulation", "halving", "bullish", "surge", "beat",
]
BEARISH = [
    "hack", "lawsuit", "ban", "crackdown", "rate hike", "inflation spike", "liquidation",
    "outflow", "recession", "bankruptcy", "bearish", "dump", "sell-off", "sanction",
]
...
GEO_WORDS = ["war", "tariff", "sanction", "china", "taiwan", "russia", "ukraine", "middle east", "oil", "election", "white house", "geopolitical"]
TRUMP_WORDS = ["trump", "donald trump", "trump administration", "white house"]
...
def _event_score(title: str, summary: str, trust: float) -> float:
    text = f"{title} {summary}".lower()
    relevance = _contains_any(text, BTC_WORDS) + _contains_any(text, MACRO_WORDS) + _contains_any(text, GEO_WORDS)
    if relevance == 0:
        return 0.0

    b = _contains_any(text, BULLISH)
    s = _contains_any(text, BEARISH)
    raw = float(b - s)

    # 트럼프/정책 키워드는 변동성 영향이 커서 가중치 부여
    trump_hits = _contains_any(text, TRUMP_WORDS)
    if trump_hits > 0:
        raw *= min(1.8, 1.0 + trump_hits * 0.25)

    # 예정된 발표(지표/FOMC/연설) 키워드가 있으면 중요도 가중
    schedule_hits = _contains_any(text, SCHEDULE_WORDS)
    if schedule_hits > 0:
        raw *= min(1.6, 1.0 + schedule_hits * 0.2)

    return raw * trust * min(2.2, 0.8 + relevance * 0.22)
...
def _iter_sources() -> List[Source]:
    ...
    # 자동 추적 쿼리 (수동 입력 없이 동작)
    auto_queries = [
        "bitcoin OR btc macro",
        "bitcoin fed rate decision",
        "bitcoin etf inflow outflow",
        "trump bitcoin",
        "trump crypto policy",
        "bitcoin war oil sanctions",
        "bitcoin middle east conflict",
        "bitcoin china taiwan tariff",
        "bitcoin russia ukraine sanctions",
        "bitcoin geopolitical risk",
        "btc risk off dollar yield",
    ]
    for q in auto_queries:
        items.append(Source(name=f"GoogleNews:{q}", url=_google_news_rss(q), kind="news", trust=0.75))
    ...
    if extra:
        for u in extra.split(","):
            ...
            if not u:
                continue
            items.append(Source(name=f"Custom:{quote(u)[:20]}", url=u, kind="news", trust=0.7))

    # 공식/신뢰 가능한 계정 RSS를 직접 주입해서 사용 (예: RSSHub/Nitter 브릿지)
    # 형식: name|url|trust ; name|url|trust
    # 예: FederalReserveX|https://.../rss|0.95;SECgovX|https://.../rss|0.9
    if tweet_rss:
        chunks = [c.strip() for c in tweet_rss.split(";") if c.strip()]
        for c in chunks:
            try:
                name, url, trust = [x.strip() for x in c.split("|", 2)]
                items.append(Source(name=name, url=url, kind="tweet", trust=float(trust)))
            except Exception:
                continue
