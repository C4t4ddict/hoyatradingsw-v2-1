...
def train_one(df: pd.DataFrame, target_col: str):
    ...
    # text/categorical -> sparse features, numeric -> dense
    text_pipe = TfidfVectorizer(max_features=4000, ngram_range=(1, 2))
    cat_pipe = Pipeline([
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ("text", text_pipe, "text"),
            ("cat", cat_pipe, ["source", "kind", "topic"]),
            ("num", num_pipe, ["score", "trust", "is_trump", "is_scheduled", "market_ret_1h", "market_ret_4h", "market_volatility_12h", "market_volume_ratio"]),
        ],
...
def main():
    ...
    for target_col in ["label_up_1h", "label_up_4h", "label_up_24h"]:
